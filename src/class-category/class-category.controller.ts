...
import { ClassCategoryDto, ClassCategoryInsertDto, LevelDto, SubjectDto } from './dtos';
...
export class ClassCategoryController {
    ...
    @ApiResponse({ status: 200, description: 'Returns subjects for the specified level' })
    ...
    @ApiParam({ name: 'subjectId', description: 'ID of the subject', type: 'string' })
    @ApiResponse({ status: 200, description: 'Returns levels for the specified subject' })
    getLevelsBySubject(@Param('subjectId') subjectId: string): Promise<LevelDto[]> {
    ...
    @ApiOperation({
        ...
        description: 'The name of the level/subject is taken first, if not present then the id will be taken',
    })
    @ApiResponse({ status: 201, description: 'Returns the created class category' })
